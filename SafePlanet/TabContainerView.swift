...
class TabContainerView: UIView, TabViewDelegate {
    ...
    @objc weak var footerBar: FooterBar?
	
    override init(frame: CGRect) {
        super.init(frame: frame)
		
        //Background image
        self.backgroundColor = Colors.radiumPink
        ...
        tabCountButton = TabCountButton().then {
            self.addSubview($0)
            $0.snp.makeConstraints { make in
                make.height.equalTo(TabContainerView.standardHeight - 3)
                make.width.equalTo(TabContainerView.standardHeight - 3)
                make.centerY.equalTo(self)
                if #available(iOS 11.0, *) {
...
//        tabCountButton.updateCount(tabList.count)
	...
	@objc func updateNavButtons() {
		...
		footerBar?.backButton?.isEnabled = tab.webContainer?.webView?.canGoBack ?? false
        footerBar?.backButton?.tintColor = (tab.webContainer?.webView?.canGoBack ?? false) ? .black : .lightGray
		footerBar?.forwardButton?.isEnabled = tab.webContainer?.webView?.canGoForward ?? false
        footerBar?.forwardButton?.tintColor = (tab.webContainer?.webView?.canGoForward ?? false) ? .black : .lightGray
	}
